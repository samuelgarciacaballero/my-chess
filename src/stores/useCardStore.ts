...
interface CardState {
  deck: Card[];
  hand: Card[];
  opponentHand: Card[]; // <— mano del rival
  ...
  markFirstCapture: () => void;
  selectCard: (id: string) => void;

  // Nuevas acciones de dev:
...
export const useCardStore = create<CardState>((set) => ({
  ...
  discardCard: (id) =>
    set((state) => ({ hand: state.hand.filter((c) => c.id !== id) })),

  markFirstCapture: () =>
    set((state) => ({
      hasFirstCapture: true,
      hand: state.initialFaceUp ? [state.initialFaceUp] : state.hand,
      initialFaceUp: null,
    })),
  ...
  drawOpponentCard: () =>
    set((state) => {
      if (state.opponentHand.length >= 3) return {}; // ya tiene 3, no roba
      const available = state.deck.filter(
